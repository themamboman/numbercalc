...
def update_cron_with_random_time():
    ...
    new_cron_command = f"{random_minute} {random_hour} * * * cd {script_dir} && python3 {os.path.join(script_dir, 'recalculate.py')}\n"
    ...
    with open(cron_file, "w") as file:
        for line in lines:
            # Remove existing entry for `recalculate.py` if it exists
            if "recalculate.py" not in line:
                file.write(line)
